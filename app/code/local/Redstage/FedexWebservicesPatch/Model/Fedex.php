...
class Redstage_FedexWebservicesPatch_Model_Fedex
...
//	implements Mage_Shipping_Model_Carrier_Interface
{
	...
	{
		parent::__construct();
		$this->_shipServiceWsdl = Mage::getModuleDir('etc', 'Redstage_FedexWebservicesPatch')  . DS . 'wsdl' . DS . 'FedEx'
			  . DS . 'ShipService_v9.wsdl';
		$this->_rateServiceWsdl = Mage::getModuleDir('etc', 'Redstage_FedexWebservicesPatch')  . DS . 'wsdl' . DS . 'FedEx'
	...
	protected function _getQuotes()
	{
		...
		$response = $this->_getCachedQuotes($requestString); // REDSTAGE COMMENTED OUT FOR 1.4.1.1 COMPATIBILITY

		$debugData = array('request' => $ratesRequest);

		if ($response === null) { // REDSTAGE COMMENTED OUT FOR 1.4.1.1 COMPATIBILITY
			try {
				$client = $this->_createRateSoapClient();
				$response = $client->getRates($ratesRequest);
				$this->_setCachedQuotes($requestString, serialize($response)); // REDSTAGE COMMENTED OUT FOR 1.4.1.1 COMPATIBILITY
				$debugData['result'] = $response;
			} catch (Exception $e) {
				$debugData['result'] = array('error' => $e->getMessage(), 'code' => $e->getCode());
				Mage::logException($e);
			}
		} else { // REDSTAGE COMMENTED OUT FOR 1.4.1.1 COMPATIBILITY
			$response = unserialize($response); // REDSTAGE COMMENTED OUT FOR 1.4.1.1 COMPATIBILITY
			$debugData['result'] = $response; // REDSTAGE COMMENTED OUT FOR 1.4.1.1 COMPATIBILITY
		} // REDSTAGE COMMENTED OUT FOR 1.4.1.1 COMPATIBILITY
	...
	protected function _getXmlQuotes()
	{
		...
		$responseBody = $this->_getCachedQuotes($request); // REDSTAGE COMMENTED OUT FOR 1.4.1.1 COMPATIBILITY
		if ($responseBody === null) { // REDSTAGE COMMENTED OUT FOR 1.4.1.1 COMPATIBILITY
			...
			try {
				...
				$this->_setCachedQuotes($request, $responseBody); // REDSTAGE COMMENTED OUT FOR 1.4.1.1 COMPATIBILITY
			}
			catch (Exception $e) {
				$debugData['result'] = array('error' => $e->getMessage(), 'code' => $e->getCode());
				$responseBody = '';
