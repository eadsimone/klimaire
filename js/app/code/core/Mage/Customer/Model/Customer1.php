...
class Mage_Customer_Model_Customer extends Mage_Core_Model_Abstract
{
    ...
    public function authenticate($login, $password)
    {
        ...
        if (!$this->validatePassword($password, $login)) {
     ...
     * @return boolean
     */
    public function validatePassword($password, $login)
    {
	 	...
	 	/*if($hash == '')
		{
			...
			->addAttributeToFilter('email', $login)
			->load();
			foreach($collection as $customer1) {
				$hash = $customer1->getPasswordHash();	
			}
			
			$hashArr = explode(':', $hash);
			$salt = $hashArr[1];
			$encPwd = md5($salt . $password);
			$encPwd = $encPwd.":".$salt;
			if($hash == $encPwd) 
			{ 
				return $hash === $encPwd;
