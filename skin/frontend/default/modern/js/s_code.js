...
s.cookieDomainPeriods=domainPeriods
...
s.linkInternalFilters="javascript:,pexsupply.com"
...
/* Plugin Example: getQueryParam 2.1
	...
	//multiple parameters
	s.campaign=s.getQueryParam('cmpid,sid',':');

	//non-page URL example
   	s.campaign=s.getQueryParam('cmpid','',document.referrer);

	//parent frame example
	s.campaign=s.getQueryParam('cmpid','','f');

	*/
	
	/* Plugin: getQueryParam v2.1 */
	if(!s.campaign)
		s.campaign=s.getQueryParam('cmpid'); /*add all query string parameters here comma delimited with cmpid*/
