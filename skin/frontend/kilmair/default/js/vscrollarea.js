...
function VSA_initScrollbars() {
    var scrollElements = VSA_getElements("scrollable5", "DIV", document, "class");
...
function VSA_handleResizeThenScroll(containerID, elID) {
    if (VSA_resizeTimer) {
        clearTimeout(VSA_resizeTimer);
        VSA_resizeTimer = 0;
    }

    scrollToMiddle(containerID, elID);

    var str = "VSA_performResizeEventThenScroll('" + containerID + "','" + elID + "')";
    VSA_resizeTimer = setTimeout(str, 100);
}
...
function VSA_performResizeEventThenScroll(containerID, elID) {
    for (var i = 0; i < VSA_scrollAreas.length; i++)
        VSA_scrollAreas[i].createScrollBar();
}
